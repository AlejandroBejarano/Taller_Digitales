...
module segments (
    ...
    // CAMBIO: Se agrega flag post_rst para inhibir las salidas seg_o y an_o
    // durante exactamente 1 ciclo despues de salir del reset. Sin este flag,
    // el always_ff de salida ejecuta con rst_i=0 y digit_sel=3 en el primer
    // ciclo post-reset (porque digit_sel se inicializa en 3 dentro del reset
    // del contador, y ese valor persiste en el primer ciclo activo). Esto
    // genera an_o=4'b0111 en lugar de 4'b1111, fallando el TEST 1 del
    // testbench que verifica los valores inmediatamente despues de rst=0.
    // post_rst es 1 exactamente durante 1 ciclo post-reset y luego vuelve a 0.
    logic post_rst;
    always_ff @(posedge clk_i) begin
        if (rst_i) post_rst <= 1'b1;
        else        post_rst <= 1'b0;
    end
    ...
    always_ff @(posedge clk_i) begin
        // CAMBIO: Se agrega condicion (rst_i || post_rst) para mantener las
        // salidas apagadas durante el reset Y el primer ciclo post-reset.
        // La condicion original solo tenia rst_i, pero en el primer ciclo
        // post-reset (cuando post_rst=1 y rst_i=0), digit_sel ya es 3 y el
        // decodificador produce an_o=4'b0111. El testbench verifica los valores
        // inmediatamente despues de rst=0, por lo que sin post_rst el TEST 1
        // fallaba con an_o=4'b0111 (esperado 4'b1111).
        if (rst_i || post_rst) begin
            seg_o <= 7'b1111111;
            an_o  <= 4'b1111;
        end else begin
